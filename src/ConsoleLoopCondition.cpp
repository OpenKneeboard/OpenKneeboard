...
static BOOL WINAPI ExitHandler(DWORD ignored) {
  OutputDebugStringA("Console exit requested.\n");
  if (SetEvent(g_ExitEvent)) {
    OutputDebugStringA("SetEVent returned true");
  }
  return TRUE;
}
...
ConsoleLoopCondition::EventType ConsoleLoopCondition::waitForSleepOrExit(
  const std::chrono::steady_clock::duration& delay) {
  int64_t expiration;
  GetSystemTimeAsFileTime((FILETIME*)&expiration);
  expiration += std::chrono::duration_cast<FILETIME_RESOLUTION>(delay).count();
  SetWaitableTimer(p->TimerEvent, (LARGE_INTEGER*)&expiration, 0, nullptr, nullptr, false);

  HANDLE handles[] = { g_ExitEvent, p->TimerEvent };
  auto res = WaitForMultipleObjects(sizeof(handles) / sizeof(handles[0]), handles, false, INFINITE);
